...
export default function DashboardLayout({ children }) {
  ...
  // Auth.js session (Google / Apple)
  const { data: nextAuthSession, status } = useSession();

  useEffect(() => {
    // Wait for Auth.js to resolve
    if (status === 'loading') return;

    // 1) Auth.js session (Google/Apple login)
    if (status === 'authenticated' && nextAuthSession?.user) {
      const u = {
        ...
        provider: nextAuthSession.user.provider || 'oauth',
        isReciclador: false,
      };
      setUser(u);
      setReady(true);
    ...
    // 2) Email/password session (localStorage)
  ...
  const logout = () => {
    ...
    // Sign out from Auth.js if using OAuth
    if (nextAuthSession) {
    ...
    return (
      <div className="loading-screen">
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--accent-primary)] to-[#27ae60] flex items-center justify-center">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M12 2a10 10 0 0 0-7.5 16.5"/><path d="M12 2c2 4 4 8 0 14"/><path d="M12 2c-2 4-4 8 0 14"/></svg>
          </div>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg-void)', display: 'flex', flexDirection: 'column' }}>
      ...
      <nav className="glass-nav fixed top-0 left-0 right-0 z-50" style={{ height: '60px' }}>
        <div className="flex items-center justify-between px-4 h-full">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(v => !v)} className="btn-ghost" style={{ padding: '8px', borderRadius: '10px', display: 'flex' }}>
          ...
          <div className="flex items-center gap-2">
            <div style={{ position: 'relative' }}>
              <button className="btn-ghost" style={{ padding: '8px', borderRadius: '10px', position: 'relative' }} onClick={() => { setNotifOpen(v => !v); setUserMenuOpen(false); }}>
                <BellIcon size={18} />
              ...
              {notifOpen && (
                <div className="glass-card" style={{ position: 'absolute', top: '46px', right: 0, width: '300px', padding: 0, overflow: 'hidden', zIndex: 100 }}>
                  ...
                  {[
                    ...
                    { title: 'Nueva alerta cerca', desc: 'Acumulación reportada a 200m', time: 'Hace 1d' },
                  ].map((n, i) => (
            ...
            <div style={{ position: 'relative' }}>
              <button className="flex items-center gap-2 btn-ghost" style={{ padding: '4px 8px 4px 4px', borderRadius: '20px' }}
      ...
      <div className="flex" style={{ paddingTop: '60px', minHeight: '100dvh' }}>
        ...
        <aside className="fixed left-0 z-40 flex flex-col"
          style={{ top: '60px', bottom: 0, width: '240px', background: 'rgba(6,14,7,0.95)', borderRight: '1px solid var(--border-glass)', backdropFilter: 'blur(30px)', transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.35s cubic-bezier(0.4,0,0.2,1)', padding: '16px 12px', overflowY: 'auto' }}>
          <div className="flex items-center gap-3 mb-6 px-2">
            {user?.image
              ? <img src={user.image} alt="" style={{ width: 38, height: 38, borderRadius: '50%', border: '2px solid var(--border-bright)' }} />
          ...
          <nav className="flex flex-col gap-1">
            {NAV_ITEMS.map(item => {
              ...
              return (
                <Link key={item.id} href={item.href} className={`sidebar-link ${isActive ? 'active' : ''}`} onClick={() => setSidebarOpen(false)} style={{ textDecoration: 'none' }}>
                  <Icon size={17} />
                  {item.label}
                </Link>
              );
      ...
      <nav className="mobile-bottom-nav">
        {NAV_ITEMS.slice(0, 5).map(item => {
          ...
          const isActive = pathname === item.href;
          return (
            <Link key={item.id} href={item.href} className={`mobile-nav-item ${isActive ? 'active' : ''}`} style={{ textDecoration: 'none' }}>
              <Icon size={22} />
